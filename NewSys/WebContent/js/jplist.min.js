...
(function() {
	var b = function(b, c) {
		...
		case "html":
			...
			break;
		case "server":
			jQuery.fn.jplist.info(a.options, "Data Source: ", a.options.dataSource),
			new jQuery.fn.jplist.ui.list.controllers.ServerHTML(a.$root, a.options, a.observer, a.panel, a.history)
		}
		var e = [],
		d = !1;
		a.options.deepLinking ? (jQuery.fn.jplist.info(a.options, "Deep linking enabled", ""), a.observer.trigger(a.observer.events.setDeepLinksEvent, [])) : (d = "cookies" === a.options.storage || "localstorage" === a.options.storage && jQuery.fn.jplist.dal.services.LocalStorageService.supported()) ? (jQuery.fn.jplist.info(a.options, "Storage enabled: ", a.options.storage), "cookies" === a.options.storage && (e = jQuery.fn.jplist.dal.services.CookiesService.restoreCookies(a.options.storageName)), "localstorage" === a.options.storage && jQuery.fn.jplist.dal.services.LocalStorageService.supported() && (e = jQuery.fn.jplist.dal.services.LocalStorageService.restore(a.options.storageName)), 0 < e.length ? a.observer.trigger(a.observer.events.restoreEvent, [e]) : a.observer.trigger(a.observer.events.forceRenderStatusesEvent, [!0])) : a.observer.trigger(a.observer.events.forceRenderStatusesEvent, [!0]);
	...
	b.prototype.findStatusByField = function(b, c) {
		for (var a = -1,
				e, d = 0; d < this.statuses.length; d++) if (e = this.statuses[d], e[c] === b[c]) {
					a = d;
					break
				}
		return a
	...
	b.prototype.getStatusesByAction = function(b) {
		for (var c = [], a, e = 0; e < this.statuses.length; e++) a = this.statuses[e],
		a.action === b && c.push(a);
		return c
	};
	...
	jQuery.fn.jplist.app.services.DTOMapperService.filters.pathGroup = function(b, f) {
	...
	jQuery.fn.jplist.app.services.DTOMapperService.filters.textFilterPathGroup = function(b, f) {
		return jQuery.fn.jplist.domain.dom.services.FiltersService.textFilterPathGroup(b.data.textAndPathsGroup, b.data.ignoreRegex, f)
	};
	...
	jQuery.fn.jplist.app.services.DTOMapperService.sort.text = function(b, f, c) {
		...
		return jQuery.fn.jplist.domain.dom.services.SortService.textHelper(f, c, b.data.order, a, b.data.ignore || "")
	};
	...
	jQuery.fn.jplist.app.services.DTOMapperService.sort.datetime = function(b, f, c) {
		var a = new jQuery.fn.jplist.domain.dom.models.DataItemMemberPathModel(b.data.path, b.data.type);
		return jQuery.fn.jplist.domain.dom.services.SortService.datetimeHelper(f, c, b.data.order, a, b.data.dateTimeFormat || "")
	}
})(); (function() {
	...
	jQuery.fn.jplist.warn = function(b, f, c) {
		jQuery.fn.jplist.logEnabled(b) && window.console.warn(f, c)
	}
})(); (function() {
	...
	var b = function(b, c, a) {
		...
		d = a.$item.find(e.jqPath),
		0 < d.length && (e = new jQuery.fn.jplist.domain.dom.models.DataItemMemberModel(d, e), b.push(e));
		a.pathitems = b;
		return jQuery.extend(this, a)
	};
	b.prototype.findPathitem = function(b) {
		for (var c = null,
				a, e = 0; e < this.pathitems.length; e++) if (a = this.pathitems[e], a.path.isEqual(b, !0)) {
					c = a;
					break
				}
		return c
	...
	var b = function(a, e) {
		...
		if (0 < d.length) {
			for (var c = 0; c < d.length; c++) if ((g = d[c]) && g.data && g.data.filterType) b = jQuery.fn.jplist.app.services.DTOMapperService.filters[g.data.filterType],
			jQuery.isFunction(b) && (a.dataview = b(g, a.dataview));
	...
	f = function(a, e) {
		var d, g = null,
		b;
		d = (new jQuery.fn.jplist.app.dto.StatusesDTOCollection(a.options, a.observer, e)).getStatusesByAction("paging", e);
		if (0 < d.length) {
			for (var c = 0; c < d.length; c++) g = d[c],
			b = g.data.currentPage || 0,
			g = new jQuery.fn.jplist.domain.dom.services.PaginationService(b, g.data.number, a.dataview.length),
			d[c].data.paging = g,
			a.dataview = jQuery.fn.jplist.domain.dom.services.FiltersService.pagerFilter(g, a.dataview);
			a.observer.trigger(a.observer.events.paginationEvent, [e, a])
		}
	},
	c = function(a, e) {
		var d;
		d = (new jQuery.fn.jplist.app.dto.StatusesDTOCollection(a.options, a.observer, e)).getStatusesByAction("sort", e);
	...
	g = function(a, e) {
		for (var d, g = -1,
				b, c = 0; c < a.dataitems.length; c++) if (d = a.dataitems[c], d = jQuery.fn.jplist.domain.dom.services.HelperService.getOuterHtml(d.jqElement), b = jQuery.fn.jplist.domain.dom.services.HelperService.getOuterHtml(e), d === b) {
					g = c;
					break
				}
		return g
	...
	m = function(a, e, d, g) {
		for (var b; d < e.length; d++) b = e.eq(d),
		h(a, b, g, d),
		d + 1 < e.length && 0 === d % 50 && window.setTimeout(function() {
			m(a, e, d, g)
		},
		0)
	},
	...
	n.prototype.dataviewToString = function() {
		var a, e = "",
		d;
		for (d = 0; d < this.dataview.length; d++) a = this.dataview[d],
		e += a.html;
		return e
	};
	...
	var b = function(b, a) {
		for (var e, d = !1,
				g = 0; g < b.paths.length; g++) if (e = b.paths[g], e.isEqual(a, !0)) {
					d = !0;
					break
				}
		return d
	...
	jQuery.fn.jplist.domain.dom.services.FiltersService.autocompleteFilter = function(b, f, c, a, e) {
		...
		if (jQuery.isNumeric(b) && jQuery.isNumeric(f) && google && jQuery.fn.jlocator && jQuery.fn.jlocator.store) {
			...
			for (var l = 0; l < a.length; l++) f = a[l],
			g = new jQuery.fn.jlocator.store(f.jqElement, {}),
			k = g.country ? jQuery.trim(g.country.toLowerCase()) == jQuery.trim(c.toLowerCase()) : !1,
					g = google.maps.geometry.spherical.computeDistanceBetween(b, g.latlng),
					(g <= e || k) && d.push(f);
			return d
		}
		return a
	...
	jQuery.fn.jplist.domain.dom.services.FiltersService.dateRangeFilter = function(b, f, c, a, e, d, g, k, l) {
		for (var h = [], m, n, p, q, r = 0; r < f.length; r++) if (m = f[r], n = m.findPathitem(b)) if (p = !jQuery.isNumeric(a) || !jQuery.isNumeric(e) || !jQuery.isNumeric(d), q = !jQuery.isNumeric(g) || !jQuery.isNumeric(k) || !jQuery.isNumeric(l), p || q) h.push(m);
		else if ((q = jQuery.fn.jplist.domain.dom.services.HelperService.formatDateTime(n.text, c)) && jQuery.isFunction(q.getFullYear)) n = new Date(a, e, d),
		p = new Date(g, k, l),
		q.setHours(0),
		q.setMinutes(0),
		q.setSeconds(0),
	...
	jQuery.fn.jplist.domain.dom.services.FiltersService.pathFilter = function(b, f) {
		for (var c, a, e = [], d = 0; d < f.length; d++) c = f[d],
		"default" === b.jqPath ? e.push(c) : (a = c.findPathitem(b)) && e.push(c);
		return e
	}
})(); (function() {
	jQuery.fn.jplist.domain.dom.services.FiltersService.pathGroupFilter = function(b, f) {
		...
		c = new jQuery.fn.jplist.domain.dom.models.DataItemMemberPathModel(c, null),
		a.push(c);
	...
	jQuery.fn.jplist.domain.dom.services.FiltersService.rangeFilter = function(b, f, c, a, e, d) {
		c = [];
		for (var g, k = jQuery.isNumeric(e), l = jQuery.isNumeric(d), h = 0; h < f.length; h++) if (a = f[h], g = a.findPathitem(b)) g = Number(g.text.replace(/[^-0-9\.]+/g, "")),
		isNaN(g) || (k && l ? g >= e && g <= d && c.push(a) : k && !l ? g >= e && c.push(a) : !k && l && g <= d && c.push(a));
		return c
	}
})(); (function() {
	jQuery.fn.jplist.domain.dom.services.FiltersService.textFilter = function(b, f, c, a) {
		for (var e, d, g = [], k, l = 0; l < c.length; l++) e = c[l],
		d = e.findPathitem(f),
		"default" === f.jqPath ? g.push(e) : d && (d = jQuery.fn.jplist.domain.dom.services.HelperService.removeCharacters(d.text, a), k = jQuery.fn.jplist.domain.dom.services.HelperService.removeCharacters(b, a), -1 !== d.indexOf(k) && g.push(e));
		return g
	}
})(); (function() {
	jQuery.fn.jplist.domain.dom.services.FiltersService.textFilterPathGroup = function(b, f, c) {
		...
		k,
		l;
		for (l = 0; l < b.length; l++) k = b[l],
		k.selected && (a = k.path, e = new jQuery.fn.jplist.domain.dom.models.DataItemMemberPathModel(a, null), k.pathObj = e, d.push(k));
		...
		for (a = 0; a < c.length; a++) {
			...
			for (var h = 0; h < d.length; h++) if (k = d[h], e = k.pathObj) if ("default" === e.jqPath) {
				l = !0;
				break
			} else if (e = b.findPathitem(e)) e = jQuery.fn.jplist.domain.dom.services.HelperService.removeCharacters(e.text, f),
			k = jQuery.fn.jplist.domain.dom.services.HelperService.removeCharacters(k.text, f),
			-1 !== e.indexOf(k) && (l = !0);
			l && g.push(b)
		}
		return g
	...
	jQuery.fn.jplist.domain.dom.services.FiltersService.textGroupFilter = function(b, f, c, a, e) {
		...
		for (var n = 0; n < e.length; n++) if (c = e[n], g = c.findPathitem(k), "default" === k.jqPath) l.push(c);
		else if (g) if (g = jQuery.fn.jplist.domain.dom.services.HelperService.removeCharacters(g.text, a), "or" === f) for (m = 0; m < b.length; m++) {
			if (d = b[m], d = jQuery.fn.jplist.domain.dom.services.HelperService.removeCharacters(d, a), -1 !== g.indexOf(d)) {
				l.push(c);
				break
			}
		} else {
			h = [];
			for (m = 0; m < b.length; m++) d = b[m],
			d = jQuery.fn.jplist.domain.dom.services.HelperService.removeCharacters(d, a),
			-1 !== g.indexOf(d) && h.push(d);
	...
	jQuery.fn.jplist.domain.dom.services.SortService.text = function(b, f, c, a) {
		c.sort(function(e, d) {
			return jQuery.fn.jplist.domain.dom.services.SortService.textHelper(e, d, b, f, a)
		})
	}
})(); (function() {
	...
	var b = function(b, a) {
		...
		if ("{month}" == b) {
			...
			if ("november" === a || "nov" === a || "nov." === a) e = 10;
	...
	f = function(b, a) {
		var e, d = null;
		e = b.replace(/{year}|{month}|{day}|{hour}|{min}|{sec}/g, ".*"); (e = RegExp(e, "g").exec(a)) && 1 < e.length && (d = e[1]);
		return d
	};
	...
	jQuery.fn.jplist.domain.dom.services.HelperService.formatDateTime = function(c, a) {
		...
		a = a.replace(/\[/g, "\\[");
				a = a.replace(/\]/g, "\\]");
				e = a.replace("{year}", "(.*)"); (d = f(e, c)) && (d = b("{year}", d));
				e = a.replace("{day}", "(.*)"); (k = f(e, c)) && (k = b("{day}", k));
				e = a.replace("{month}", "(.*)"); (g = f(e, c)) && (g = b("{month}", g));
				e = a.replace("{hour}", "(.*)"); (l = f(e, c)) && (l = b("{hour}", l));
				e = a.replace("{min}", "(.*)"); (h = f(e, c)) && (h = b("{min}", h));
				e = a.replace("{sec}", "(.*)"); (e = f(e, c)) && (e = b("{sec}", e));
				if (!d || isNaN(d)) d = 1900;
				if (!g || isNaN(g)) g = 0;
				if (!k || isNaN(k)) k = 1;
				if (!l || isNaN(l)) l = 0;
	...
	jQuery.fn.jplist.animation.drawItems = function(b, f, c, a, e, d, g, k) {
		...
		if (e = jQuery.fn.jplist.animation[e]) {
			...
			if (jQuery.isFunction(m)) k.on(k.events.animationStepEvent,
					function(e, d, g) {
				m(b, f, c, a, d)
			});
			k.on(k.events.animationCompleteEvent,
					function(e) {
	...
	f = function(a, e) {
		jQuery.isNumeric(e) && 0 < e ? a.handler = window.setTimeout(function() {
			...
			a.observer.trigger(a.observer.events.animationStepEvent, [100 * a.progress, a]);
	...
	f = function(a, d, g) {
		...
		0 >= c.length || 0 >= f.length ? (a.$noResults.removeClass("jplist-hidden"), a.$itemsBox.addClass("jplist-hidden"), b(a, d, g, f)) : (a.$noResults.addClass("jplist-hidden"), a.$itemsBox.removeClass("jplist-hidden"), a.options.effect ? (a.history && (m = a.history.getLastStatus()) && !m.inAnimation && (h = !0), h = h ? n: a.options, jQuery.fn.jplist.animation.drawItems(h, a.$itemsBox, c, f, a.options.effect, a.timeline,
				function() {
			b(a, d, g, f)
		},
	...
	c = function(a) {
		a.observer.on(a.observer.events.renderList,
				function(d, g, b) {
			f(a, g, b)
		})
	},
	...
	var b = function(b) {
		b.observer.on(b.observer.events.renderStatusesEvent,
				function(a, e) {
			b.storage.save(e);
	...
	var b = function(b) {
		b.observer.on(b.observer.events.renderList,
				function(a, e, d) {
			b.storage.save(d);
			jQuery.fn.jplist.dal.services.URIService.get(d, b.options,
					function(a, e) {
				...
				for (var n = 0; n < m.length; n++) f = m[n],
				f.data.currentPage || (f.data.currentPage = 0),
				f = new jQuery.fn.jplist.domain.dom.services.PaginationService(f.data.currentPage, f.data.number, d.count),
				m[n].data.paging = f;
				b.model.set(d, e)
			},
			function(a) {},
	...
	f = function(a, d, g) {
		...
		a.options.effect ? (a.history && (f = a.history.getLastStatus()) && !f.inAnimation && (c = !0), c = c ? h: a.options, jQuery.fn.jplist.animation.drawItems(c, a.$itemsBox, null, jQuery(d), a.options.effect, a.timeline,
				function() {
			b(a, d, g)
		},
	...
	c = function(a) {
		a.observer.on(a.observer.events.renderStatusesEvent,
				function(d, g) {
			a.$noResults.addClass("jplist-hidden");
			a.observer.trigger(a.observer.events.renderList, [a, g])
		});
		a.scopeObserver.on(a.scopeObserver.events.modelChanged,
				function(d, g, b) {
	...
	c = function(a) {
		a.observer.on(a.observer.events.setStatusesEvent,
				function(e, b, c) {
			a.controls.setStatuses(b, !1);
			a.history.addList(b);
			jQuery.fn.jplist.info(a.options, "Set statuses event: ", b)
		});
		a.observer.on(a.observer.events.forceRenderStatusesEvent,
				function(e, b) {
			var c;
		...
		a.observer.on(a.observer.events.restoreEvent,
				function(e, b) {
		...
		a.observer.on(a.observer.events.changeUrlDeepLinksEvent,
				function(e) {
			e = "";
		...
		a.observer.on(a.observer.events.setDeepLinksEvent,
				function(e) {
			...
			if (a.options.deepLinking && "" !== jQuery.trim(b)) {
				c = b.split(a.options.delimiter1);
				for (b = 0; b < c.length; b++) f = c[b],
				n = f.split("="),
				2 === n.length && (f = n[0], n = n[1], f = f.split(a.options.delimiter0), 2 === f.length && (f = {
						controlName: f[0],
						propName: f[1],
						propValue: n
				},
			...
			0 >= m.length ? (c = "cookies" === a.options.storage || "localstorage" === a.options.storage && jQuery.fn.jplist.dal.services.LocalStorageService.supported()) ? ("cookies" === a.options.storage && (e = jQuery.fn.jplist.dal.services.CookiesService.restoreCookies(a.options.storageName)), "localstorage" === a.options.storage && jQuery.fn.jplist.dal.services.LocalStorageService.supported() && (e = jQuery.fn.jplist.dal.services.LocalStorageService.restore(a.options.storageName)), 0 < e.length ? a.observer.trigger(a.observer.events.restoreEvent, [e]) : a.observer.trigger(a.observer.events.forceRenderStatusesEvent, [!1])) : a.observer.trigger(a.observer.events.forceRenderStatusesEvent, [!0]) : a.controls.setDeepLinks(m)
		});
	...
	a = function(a) {
		...
		jQuery.fn.jplist.logEnabled(a.options) && (e = jQuery.map(a.paths,
				function(a, e) {
	...
	b.prototype.create = function(b) {
		...
		k = g = d = !0;
		l = !1;
		m = h = null; (e = b.attr("data-control-deep-link")) && "false" === e.toString() && (d = !1); (e = b.attr("data-control-storage")) && "false" === e.toString() && (g = !1); (e = b.attr("data-control-animation")) && "false" === e.toString() && (k = !1); (e = b.attr("data-control-animate-to-top")) && "true" === e.toString() && (l = !0);
		if (e = this.options.controlTypes[a]) e.className && (h = jQuery.fn.jplist.ui.controls[e.className]),
	...
	f = function(a, d) {
		for (var g, b = 0; b < a.length; b++) g = a[b],
		g.name === d.name && g.action === d.action && jQuery.extend(!0, g, d);
		return a
	},
	c = function(a, d) {
	...
	a = function(a, d, g, b, c) {
		...
		for (a = 0; a < c.$controls.length; a++) g = c.$controls.eq(a),
		d = c,
		(g = d.controlFactory.create(g)) && d.controls.push(g);
		return jQuery.extend(this, c)
	};
	...
	a.prototype.setDeepLinks = function(a) {
		...
		for (h = 0; h < a.length; h++) {
			...
			var m = [];
			b = void 0;
			for (var n = 0; n < this.controls.length; n++) b = this.controls[n],
			b.name === g && m.push(b);
			g = m;
			for (m = 0; m < g.length; m++) b = g[m],
			jQuery.isFunction(b.getStatusByDeepLink) && (b = b.getStatusByDeepLink(d.propName, d.propValue)) && l.push(b)
		}
		a = c(this, !1);
	...
	a.prototype.setStatuses = function(a, d) {
		for (var b, c, f = 0; f < a.length; f++) {
			...
			c = b.name;
			for (var h = b.action,
					m = [], n = void 0, p = 0; p < this.controls.length; p++) n = this.controls[p],
					n.name === c && n.action === h && m.push(n);
			c = m;
			for (h = 0; h < c.length; h++) jQuery.isFunction(c[h].setStatus) && c[h].setStatus(b, d)
		}
	};
	a.prototype.getDeepLinksUrl = function() {
		var a;
		a = "";
		for (var d = [], b = "", c = 0; c < this.controls.length; c++) a = this.controls[c],
		jQuery.isFunction(a.getDeepLink) && (b = jQuery.trim(a.getDeepLink())),
		"" !== b && -1 === jQuery.inArray(b, d) && d.push(b);
	...
	a.prototype.getPaths = function() {
		var a, d = [],
		g;
		g = new b.dom.collections.DataItemMemberPathCollection(this.options, this.observer);
		for (var c = 0; c < this.controls.length; c++) a = this.controls[c],
		jQuery.isFunction(a.getPaths) && (a.getPaths(d), g.addRange(d));
		return g.paths
	};
	a.prototype.add = function(a) { (a = this.controlFactory.create(a)) && this.controls.push(a)
	};
	...
	jQuery.fn.jplist.dal.services.CookiesService.getCookie = function(b) {
		...
		for (var d = 0; d < a.length; d++) if (f = a[d].substr(0, a[d].indexOf("=")), c = a[d].substr(a[d].indexOf("=") + 1), f = f.replace(/^\s+|\s+$/g, ""), f == b) {
			e = unescape(c);
			break
		}
		return e
	...
	jQuery.fn.jplist.dal.services.CookiesService.restoreCookies = function(b) {
		var f = []; (b = jQuery.fn.jplist.dal.services.CookiesService.getCookie(b)) && (f = jQuery.parseJSON(b));
		f || (f = []);
		return f
	}
})(); (function() {
	...
	jQuery.fn.jplist.dal.services.LocalStorageService.restore = function(b) {
		var f = []; (b = window.localStorage[b]) && (f = jQuery.parseJSON(b));
		f || (f = []);
		return f
	}
})(); (function() {
	...
	jQuery.fn.jplist.dal.services.URIService.get = function(b, f, c, a, e) {
		...
		d.ajax.data.statuses = encodeURIComponent(JSON.stringify(b,
				function(a, e) {
	...
	var b = function(a, d) {
		...
		case "paging":
			...
			break;
		case "sort":
			(b = a.$control.attr("data-datetime-format")) || (b = ""); (f = a.$control.attr("data-ignore")) || (f = "");
			b = new jQuery.fn.jplist.ui.controls.DropdownSortDTO(c.attr("data-path"), c.attr("data-type"), c.attr("data-order"), b, f);
			b = new jQuery.fn.jplist.app.dto.StatusDTO(a.name, a.action, a.type, b, a.inStorage, a.inAnimation, a.isAnimateToTop, a.inDeepLinking);
			break;
		case "filter":
			b = new jQuery.fn.jplist.ui.controls.DropdownFilterDTO(c.attr("data-path"), c.attr("data-type")),
			b = new jQuery.fn.jplist.app.dto.StatusDTO(a.name, a.action, a.type, b, a.inStorage, a.inAnimation, a.isAnimateToTop, a.inDeepLinking)
		}
		return b
	},
	...
	a.prototype.setStatus = function(a, d) {
		var b;
		b = null;
		this.inStorage && d && this.$control.data("storage-status", a); ! this.inStorage && d && (b = this.$control.find('li:has(span[data-default="true"])').eq(0), 0 >= b.length && (b = this.$control.find("li:eq(0)")), b = b.find("span"));
		switch (this.action) {
		case "filter":
			!this.inStorage && d && 0 < b.length ? (a.data.path = b.attr("data-path"), a.data.type = b.attr("data-type"), this.history.addStatus(a), this.observer.trigger(this.observer.events.statusEvent, [a])) : (this.$control.find("li").removeClass("active"), b = this.$control.find('li:has([data-path="' + a.data.path + '"])'), b.addClass("active"), this.$control.find(".jplist-dd-panel").text(b.eq(0).text()));
			break;
		case "sort":
	...
	var b = function(a, d) {
		...
		case "sort":
			...
			break;
		case "filter":
			c = new jQuery.fn.jplist.ui.controls.DropdownFilterDTO(b.attr("data-path"), b.attr("data-type")),
			c = new jQuery.fn.jplist.app.dto.StatusDTO(a.name, a.action, a.type, c, a.inStorage, a.inAnimation, a.isAnimateToTop, a.inDeepLinking)
		}
		return c
	},
	...
	a.prototype.setStatus = function(a, b) {
		...
		switch (this.action) {
		case "filter":
			if (!this.inStorage && b) a.data.path = "default",
			this.history.addStatus(a),
			...
			break;
		case "sort":
			!this.inStorage && b ? (a.data.path = "default", this.history.addStatus(a), this.observer.trigger(this.observer.events.statusEvent, [a])) : (c = "default" == a.data.path ? this.$control.find('option[data-path="' + a.data.path + '"]') : this.$control.find('option[data-path="' + a.data.path + '"][data-type="' + a.data.type + '"][data-order="' + a.data.order + '"]'), 0 < c.length && (c.get(0).selected = !0));
			break;
		case "paging":
	...
	c.prototype.getStatusByDeepLink = function(a, e) {
		...
		a: if (d = null, this.inDeepLinking) {
			...
			d = b(this, !0);
			d.data && "currentPage" === a && (d.data.currentPage = e)
		}
		return d
	};
	c.prototype.setStatus = function(a, b) {
		this.inStorage && b && this.$control.data("storage-status", a); ! this.inStorage && b ? (a.data.currentPage = 0, this.history.addStatus(a), this.observer.trigger(this.observer.events.statusEvent, [a])) : a.data && a.data.paging && this.params.view.build(a.data.paging)
	};
	...
	var b = function(b, a, e) {
		...
		for (var g = b; g < a; g++) d += '<button type="button" data-type="page" ',
		g === e && (d += ' class="jplist-current" data-active="true" '),
		b = g + 1,
		d += ' data-number="' + g + '" ',
		d += ">" + b + "</button> ";
		return d + "</div>"
	...
	f = function(b, a) {
		...
		f,
		l;
		d = e.$control.attr("data-prev") || e.options.prevArrow;
		...
		e.$pagingnext = e.$control.find('[data-type="pagingnext"]');
	...
	f.prototype.build = function(c) {
		if (0 <= c.currentPage && c.currentPage < c.pagesNumber) {
			...
			1 >= c.pagesNumber ? this.$control.addClass("jplist-one-page") : this.$control.removeClass("jplist-one-page")
	...
	b.prototype.setStatus = function(b, c) {
		var a, e;
		a = b.data.paging; ! a || 0 >= a.pagesNumber ? (this.$control.html(""), this.$control.addClass("jplist-empty")) : (this.$control.removeClass("jplist-empty"), e = this.$control.attr("data-type"), e = e.replace("{current}", a.currentPage + 1), e = e.replace("{pages}", a.pagesNumber), e = e.replace("{start}", a.start + 1), e = e.replace("{end}", a.end), e = e.replace("{all}", a.itemsNumber), this.$control.html(e))
	};
	jQuery.fn.jplist.ui.controls.PaginationInfo = function(f) {
		return new b(f)
	}
})(); (function() {
	var b = function(b) {
	...
	f = function(a) {
		...
		else a.controlOptions && a.controlOptions.eventName && (e = a.controlOptions.eventName),
		a.$control.on(e,
				function() {
			a.history.addStatus(b(a, !1));
			a.observer.trigger(a.observer.events.forceRenderStatusesEvent, [!1])
	...
	c.prototype.setStatus = function(a, b) {
		this.inStorage && b && this.$control.data("storage-status", a); ! this.inStorage && b ? (a.data.value = "", this.observer.trigger(this.observer.events.statusEvent, [a])) : a.data && (a.data.value || (a.data.value = ""), this.$control.val(a.data.value))
	};
	...
	c.prototype.setStatus = function(a, b) {
		a.data && (this.inStorage && b && (this.params.storageStatus = a), this.$root.removeClass(this.params.types.join(" ")), !this.inStorage && b ? (this.$root.addClass(this.params.defaultView), a.data.view = this.params.defaultView, this.params.currentView = this.params.defaultView, this.observer.trigger(this.observer.events.statusEvent, [a])) : (this.$root.addClass(a.data.view), this.params.currentView = a.data.view))
	};
	...
	var b = function(a, b) {
		...
		case "text":
			...
			break;
		case "range":
			h = jQuery.fn.jplist.domain.dom.services.FiltersService.rangeFilter(c, b, 0, 0, Number(a.$control.data("dataMin")), Number(a.$control.data("dataMax")))
		}
	...
	f = function(a) {
		...
		case "static":
			a.observer.on(a.observer.events.collectionReadyEvent,
					function(e, d) {
				b(a, d.dataitems)
			});
			break;
		case "filter":
			a.observer.on(a.observer.events.filterEvent,
					function(e, d, c) {
				b(a, c.dataview)
			});
			break;
	...
	c = function(a) {
		...
		d && (b = new jQuery.fn.jplist.domain.dom.models.DataItemMemberPathModel(d, null), a.$control.data("path", b));
	...
	c.prototype.getStatusByDeepLink = function(a, b) {
		...
		case "next":
			c = b.split(this.options.delimiter2),
			3 === c.length && (d.data.next_year = c[0], d.data.next_month = c[1], d.data.next_day = c[2])
		}
		return d
	};
	c.prototype.getPaths = function(a) {
		var b;
		if (b = this.$control.attr("data-path").toString()) b = new jQuery.fn.jplist.domain.dom.models.DataItemMemberPathModel(b, "datetime"),
		a.push(b)
	};
	c.prototype.setStatus = function(a, b) {
		var d, c, f;
		...
		jQuery.isNumeric(a.data.next_year) && jQuery.isNumeric(a.data.next_month) && jQuery.isNumeric(a.data.next_day) ? (d = new Date(a.data.next_year, a.data.next_month, a.data.next_day), f.datepicker("setDate", d)) : f.val("")
	};
	...
	c = function(a) {
		...
		a.params.controlOptions && (a.params.controlOptions.ui_slider(a.params.$uiSlider, a.params.$prev, a.params.$next), a.params.controlOptions.set_values(a.params.$uiSlider, a.params.$prev, a.params.$next));
		f(a);
		return jQuery.extend(this, a)
	};
	c.prototype.getStatus = function(a) {
	...
	c.prototype.getPaths = function(a) {
		var b;
		if (b = this.$control.attr("data-path").toString()) b = new jQuery.fn.jplist.domain.dom.models.DataItemMemberPathModel(b, "number"),
		a.push(b)
	};
	c.prototype.setStatus = function(a, b) {
		var d, c;
		this.inStorage && b && (this.params.storageStatus = a);
		jQuery.isNumeric(a.data.prev) && jQuery.isNumeric(a.data.next) && (d = Number(a.data.prev), c = Number(a.data.next), !isNaN(d) && !isNaN(c) && (this.params.$uiSlider.slider("values", 0) != d && this.params.$uiSlider.slider("values", 0, d), this.params.$uiSlider.slider("values", 1) != c && this.params.$uiSlider.slider("values", 1, c)));
	...
	c.prototype.getStatusByDeepLink = function(a, c) {
		var d = null,
		f;
		if (this.inDeepLinking && (d = b(this, !1), d.data && (f = "selected" === a && "true" === c))) d.data.selected = f,
		d.data.filterType = "path";
		return d
	};
	c.prototype.getPaths = function(a) {
		var b;
		if (b = this.$control.attr("data-path")) b = new jQuery.fn.jplist.domain.dom.models.DataItemMemberPathModel(b, "text"),
		a.push(b)
	};
	c.prototype.setStatus = function(a, b) {
	...
	f = function(a, b) {
		...
		a.params.$buttons.each(function(a, e) {
			if (d = jQuery(e).attr("data-path")) c = new jQuery.fn.jplist.domain.dom.models.DataItemMemberPathModel(d, "text"),
			b.push(c)
		})
	},
	c = function(a, b, d) {
		...
		if (b.data && b.data.pathGroup && jQuery.isArray(b.data.pathGroup) && 0 < b.data.pathGroup.length) for (var e = 0; e < b.data.pathGroup.length; e++) d = b.data.pathGroup[e],
		c = a.params.$buttons.filter('[data-path="' + d + '"]'),
		0 < c.length && (c.addClass("jplist-selected"), c.data("selected", !0))
	},
	a = function(a) {
		a.params.$buttons.on("click",
				function() {
	...
	e = function(a) {
		...
		"multiple" === a.params.mode ? a.params.$buttons.each(function() {
			...
			b.data("selected", d)
		}) : (a.params.$buttons.data("selected", !1), b = a.params.$buttons.filter('[data-selected="true"]'), b = 0 < b.length ? b.eq(0) : a.params.$buttons.eq(0), b.data("selected", !0), b.attr("data-selected", !0), b.trigger("click"))
	},
	d = function(b) {
		b.params = {
	...
	d.prototype.getDeepLink = function() {
		...
		if (this.inDeepLinking && (c = b(this, !1), c.data && c.data.pathGroup && 0 < c.data.pathGroup.length)) {
			for (var e = 0; e < c.data.pathGroup.length; e++) a = c.data.pathGroup[e],
			0 < e && (d += this.options.delimiter2),
			d += a;
	...
	c.prototype.getStatusByDeepLink = function(a, c) {
		...
		f;
		if (this.inDeepLinking && (d = b(this, !1), d.data && (f = "selected" === a && "true" === c))) d.data.selected = f,
		d.data.value = this.params.dataText;
		return d
	};
	c.prototype.getPaths = function(a) {
		var b;
	...
	var b = function(a, b) {
		...
		}), c = new jQuery.fn.jplist.ui.controls.ButtonTextFilterGroupDTO(e, c), c = new jQuery.fn.jplist.app.dto.StatusDTO(a.name, a.action, a.type, c, a.inStorage, a.inAnimation, a.isAnimateToTop, a.inDeepLinking));
		return c
	},
	f = function(a, b) {
		var c, e, f;
		a.params.$buttons.each(function(a, d) {
			c = jQuery(this);
			if (e = c.attr("data-path")) f = new jQuery.fn.jplist.domain.dom.models.DataItemMemberPathModel(e, "text"),
			b.push(f)
		})
	},
	c = function(a, b, c) {
		...
		if (b.data && b.data.textAndPathsGroup && jQuery.isArray(b.data.textAndPathsGroup) && 0 < b.data.textAndPathsGroup.length) for (var f = 0; f < b.data.textAndPathsGroup.length; f++) c = b.data.textAndPathsGroup[f],
		e = a.params.$buttons.filter('[data-path="' + c.path + '"][data-text="' + c.text + '"]'),
		0 < e.length && c.selected && (e.addClass("jplist-selected"), e.data("selected", !0))
	},
	a = function(a) {
		var c;
		a.params.$buttons.on("click",
	...
	e.prototype.getDeepLink = function() {
		...
		if (this.inDeepLinking && (c = b(this, !1), c.data && c.data.textAndPathsGroup && 0 < c.data.textAndPathsGroup.length)) {
			for (var h = 0; h < c.data.textAndPathsGroup.length; h++) e = c.data.textAndPathsGroup[h],
			e.selected && f.push(e.text + this.options.delimiter3 + e.path);
	...
	e.prototype.getStatusByDeepLink = function(a, c) {
		...
		if (this.inDeepLinking && (e = b(this, !1), e.data && "selected" === a)) {
			...
			for (var m = 0; m < f.length; m++) h = f[m].split(this.options.delimiter3),
			2 === h.length && e.data.textAndPathsGroup.push({
				selected: !0,
				text: h[0],
				path: h[1]
			})
	...
	var b = function(a, b) {
		...
		c = new jQuery.fn.jplist.ui.controls.CheckboxGroupFilterDTO(e),
		c = new jQuery.fn.jplist.app.dto.StatusDTO(a.name, a.action, a.type, c, a.inStorage, a.inAnimation, a.isAnimateToTop, a.inDeepLinking);
		return c
	},
	f = function(a, b) {
		a.params.$checkboxes.each(function(a, c) {
			var d;
			if (d = jQuery(this).attr("data-path")) d = new jQuery.fn.jplist.domain.dom.models.DataItemMemberPathModel(d, "text"),
			b.push(d)
		})
	},
	c = function(a, b, c) {
		...
		if (b.data && b.data.pathGroup && jQuery.isArray(b.data.pathGroup) && 0 < b.data.pathGroup.length) for (var f = 0; f < b.data.pathGroup.length; f++) c = b.data.pathGroup[f],
		e = a.params.$checkboxes.filter('[data-path="' + c + '"]'),
		0 < e.length && (e.addClass("jplist-selected"), e.get(0).checked = !0)
	},
	a = function(a) {
	...
	e.prototype.getDeepLink = function() {
		...
		if (this.inDeepLinking && (c = b(this, !1), c.data && jQuery.isArray(c.data.pathGroup) && 0 < c.data.pathGroup.length)) {
			for (a = 0; a < c.data.pathGroup.length; a++)"" !== e && (e += this.options.delimiter2),
			e += c.data.pathGroup[a];
	...
	var b = function(a, b) {
		...
		if (!b || !c) a.params.$checkboxes.each(function(a, c) {
			var e = jQuery(c);
			l = b ? e.data("selected-on-start") || !1 : e.get(0).checked; (e = e.val()) && l && f.push(e)
		}),
		c = new jQuery.fn.jplist.ui.controls.CheckboxTextFilterDTO(f, a.params.dataLogic, a.params.dataPath, a.params.ignore),
		c = new jQuery.fn.jplist.app.dto.StatusDTO(a.name, a.action, a.type, c, a.inStorage, a.inAnimation, a.isAnimateToTop, a.inDeepLinking);
		return c
	},
	f = function(a, b, c) {
		...
		if (b.data && b.data.textGroup && jQuery.isArray(b.data.textGroup) && 0 < b.data.textGroup.length) for (var l = 0; l < b.data.textGroup.length; l++) c = b.data.textGroup[l],
		f = a.params.$checkboxes.filter('[value="' + c + '"]'),
		0 < f.length && (f.addClass("jplist-selected"), f.get(0).checked = !0)
	},
	c = function(a) {
		a.params.$checkboxes.on("change",
				function() {
	...
	a.prototype.getDeepLink = function() {
		...
		if (this.inDeepLinking && (c = b(this, !1), c.data && jQuery.isArray(c.data.textGroup) && 0 < c.data.textGroup.length)) {
			for (a = 0; a < c.data.textGroup.length; a++)"" !== f && (f += this.options.delimiter2),
			f += c.data.textGroup[a];
	...
	var b = function(a, b) {
		...
		if (!b || !c) f = b ? a.params.initialSelected || !1 : a.$control.get(0).checked,
				c = {
				path: a.$control.attr("data-path"),
				type: "TextFilter",
				filterType: "path",
	...
	f = function(a) {
		...
		a.$control.on("click",
			...
			if ((f = a.history.getLastList() || []) && c) {
				for (var k = 0; k < f.length; k++) d = f[k],
				d.name === c.name && d.action === c.action && jQuery.extend(!0, f[k], c);
	...
	var b = function(b) {
		...
		b.observer.on(b.observer.events.setStatusesEvent,
				function(a, e, d) {
