...
jBoxConfig.defaults = {
    ...
    border: 10, /* 窗口的外边框像素大小,必须是0以上的整数 */
    opacity: 0.8, /* 窗口隔离层的透明度,如果设置为0,则不显示隔离层 */
    ...
    title: '信息提示', /* 窗口的标题 */
    width: '70%', /* 窗口的宽度，值为'auto'或表示像素的整数 */
    height: 'auto', /* 窗口的高度，值为'auto'或表示像素的整数 */
    bottomText: '', /* 窗口的按钮左边的内容，当没有按钮时此设置无效 */
    buttons: { '确定': 'ok' }, /* 窗口的按钮 */
    buttonsFocus: 0, /* 表示第几个按钮为默认按钮，索引从0开始 */
    loaded: function (h) {$("body").css('overflow','hidden');$('body').bind("touchmove",function(e){e.preventDefault();});}, /* 窗口加载完成后执行的函数，需要注意的是，如果是ajax或iframe也是要等加载完http请求才算窗口加载完成，参数h表示窗口内容的jQuery对象 */
    submit: function (v, h, f) { return true; }, /* 点击窗口按钮后的回调函数，返回true时表示关闭窗口，参数有三个，v表示所点的按钮的返回值，h表示窗口内容的jQuery对象，f表示窗口内容里的form表单键值 */
    closed: function () {$("body").css('overflow','auto');$("body").unbind("touchmove");} /* 窗口关闭后执行的函数 */
};
...
jBoxConfig.tipDefaults = {
    ...
    closed: function () {$("body").css('overflow','auto');$("body").unbind("touchmove");} /* 提示关闭后执行的函数 */
};
...
jBoxConfig.messagerDefaults = {
    ...
    buttons: {}, /* 信息的按钮 */
    buttonsFocus: 0, /* 表示第几个按钮为默认按钮，索引从0开始 */
    loaded: function (h) {$("body").css('overflow','hidden');$('body').bind("touchmove",function(e){e.preventDefault();});}, /* 窗口加载完成后执行的函数，参数h表示窗口内容的jQuery对象 */
    submit: function (v, h, f) { return true; }, /* 点击信息按钮后的回调函数，返回true时表示关闭窗口，参数有三个，v表示所点的按钮的返回值，h表示窗口内容的jQuery对象，f表示窗口内容里的form表单键值 */
    closed: function () {$("body").css('overflow','auto');$("body").unbind("touchmove");} /* 信息关闭后执行的函数 */
};
