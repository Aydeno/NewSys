...
import com.tools.HibernateHqlUtils;
import com.tools.Page;
import com.tools.PropertyFilter;
import com.tools.PropertyFilter.MatchType;
import com.tools.ReflectionUtils;



/**
 * 封装扩展功能的Hibernate泛型基类.
 * 
...
public class HibernateDao<T, PK extends Serializable> extends SimpleHibernateDao<T, PK> {
	...
	public Page<T> find(final Page<T> page, String hql, final Object... values) {
		...
		if (page.isAutoCount()) {			
			String countQueryString = " select count (*) " + HibernateHqlUtils.removeSelect(HibernateHqlUtils.removeOrders(hql));
			Query q = createQuery(countQueryString,values); 
			Long count = (Long)q.uniqueResult();
			page.setTotalCount(count.intValue());
        }
		if (page.isOrderBySetted()) {
        	//去掉order by 条件
        	hql = HibernateHqlUtils.removeOrders(hql);

    		if (page.getOrder().endsWith(page.ASC)) {
				hql = hql + " order by "+Order.asc(page.getOrderBy());		
			} else {
				hql = hql + " order by "+Order.desc(page.getOrderBy());
			}    		
		}
	...
	*/
	 ...
	 * 按属性查找对象列表,支持多种匹配方式.
	 * 
	 * @param matchType 目前支持的取值为"EQ"与"LIKE".
	 */
	...
	protected Criterion[] buildFilterCriterions(final List<PropertyFilter> filters) {
		...
		for (PropertyFilter filter : filters) {
			...
			if (!multiProperty) { //properName中只有一个属性的情况.						
	...
	protected Criterion buildPropertyCriterion(final String propertyName, final Object value, final MatchType matchType) {
		...
		if (MatchType.GREATER.equals(matchType)) {
			criterion = Restrictions.gt(propertyName, value);
		}
		if (MatchType.LESS.equals(matchType)) {
			criterion = Restrictions.lt(propertyName, value);
		}
		if (MatchType.GREATEREQ.equals(matchType)) {
			criterion = Restrictions.ge(propertyName, value);
		}
		if (MatchType.LESSEQ.equals(matchType)) {
